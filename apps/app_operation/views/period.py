...
def period_detail_view(request, period_pk):
    ...
    with DebugContext.section("Fetching financial period", {"period_pk": period_pk, "user": request.user.username}):
        period = get_object_or_404(
            FinancialPeriod,
            pk=period_pk,
            error_message="Financial period not found or has been deleted."
        )
        DebugContext.success("Period loaded", {
            "period_id": period.pk,
...
def period_list_view(request, entity_pk):
    ...
    with DebugContext.section("Fetching entity and periods", {"entity_pk": entity_pk, "user": request.user.username}):
        ...
        DebugContext.success("Periods loaded", {
            "entity_id": entity.pk,
...
def period_create_view(request, entity_pk):
    ...
    with DebugContext.section("Fetching entity for period creation", {"entity_pk": entity_pk, "user": request.user.username}):
        entity = get_object_or_404(
            Entity,
            pk=entity_pk,
            error_message="Entity not found or has been deleted."
    ...
    if request.method == "POST":
        with DebugContext.section("Processing period creation", {
            "entity_pk": entity_pk,
            ...
            try:
                ...
                with DebugContext.section("Creating financial period", {"start_date": start_date}):
                    period = FinancialPeriod(entity=entity, start_date=start_date)
                    period.full_clean()
                    period.save()

                    DebugContext.success("Period created successfully", {
                        "period_id": period.pk,
                        "start_date": str(period.start_date),
                    })
                    DebugContext.audit(
...
def period_close_view(request, period_pk):
    ...
    with DebugContext.section("Fetching period for closing", {"period_pk": period_pk, "user": request.user.username}):
        period = get_object_or_404(
            FinancialPeriod,
            pk=period_pk,
            error_message="Financial period not found or has been deleted."
    ...
    if period.end_date is not None:
        ...
        DebugContext.warn("Close attempt on already closed period", {"period_id": period.pk})
        DebugContext.audit(
    ...
    if request.method == "POST":
        with DebugContext.section("Processing period closure", {
            "period_pk": period_pk,
            "user": request.user.username,
        }):
            try:
                ...
                with transaction.atomic():
                    with DebugContext.section("Closing period", {"end_date": end_date}):
                        ...
                        DebugContext.success("Period closed successfully", {
                            "period_id": period.pk,
                            "end_date": str(period.end_date),
                        })
                        DebugContext.audit(
