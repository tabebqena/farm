...
def record_transaction_repayment(request, pk):
    ...
    with DebugContext.section(
        "Validating repayment eligibility",
        {
            "operation_id": operation.pk,
            "operation_type": operation.operation_type,
        },
        ...
        if not proxy_cls:
            ...
            DebugContext.error(
                error_msg, None, {"operation_type": operation.operation_type}
            )
            DebugContext.audit(
        ...
        if not data.get("has_repayment"):
            ...
            return HttpResponseBadRequest(error_msg)
        DebugContext.success(
            "Repayment validation passed", {"operation_id": operation.pk}
        )
        ...
        with DebugContext.section(
            ...
            if form.is_valid():
                if form.cleaned_data["amount"] > remaining_amount:
                    error_msg = (
                        f"Amount exceeds remaining balance of {remaining_amount}."
                    )
                    form.add_error("amount", error_msg)
                    DebugContext.warn(
                        "Repayment amount exceeds remaining balance",
                        {
                            "requested_amount": str(form.cleaned_data.get("amount", 0)),
