...
class CapitalLossCreateTest(TestCase):
    ...
    def test_source_must_be_project_entity(self):
        non_project_person = Person.create(private_name="Regular Person")
        op = self._make_op(source=non_project_person.entity)
    ...
    def test_destination_is_immutable(self):
        ...
        other_system = Entity.create(is_system=True)

        op.destination = other_system
        with self.assertRaises(ValidationError):
            op.save()
