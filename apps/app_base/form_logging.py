...
class LoggingFormMixin:
    ...
    def clean(self):
        ...
        DebugContext.log(f"{form_name}.clean()", {
            "form": form_name,
        ...
        try:
            ...
            if not self.errors:
                DebugContext.success(f"{form_name} validation passed", {
    ...
    def clean_field(self, field_name: str):
        ...
        DebugContext.log(f"Validating {self.__class__.__name__}.{field_name}", {
            "field": field_name,
    ...
    def save(self, commit=True):
        ...
        with DebugContext.section(f"{form_name}.save() ({action})", {
            "form": form_name,
            "is_new": is_new,
            "instance_pk": self.instance.pk if self.instance and self.instance.pk else None,
            "commit": commit,
            ...
            try:
                ...
                DebugContext.success(f"{form_name} saved", {
                ...
                if commit:
                    DebugContext.audit(
                        ...
                        entity_type=self._meta.model.__name__ if hasattr(self, '_meta') and self._meta else "Unknown",
