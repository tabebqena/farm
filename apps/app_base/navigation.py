...
NAVIGATION_MAP = {
    ...
    "operation_detail_view": {
        ...
        "related": [
            ("Source Entity", "entity_detail", {"pk": "pk"}),
            ("Destination Entity", "entity_detail", {"pk": "pk"}),
...
def get_navigation_context(
    current_view_name, view_kwargs, entity=None, person_pk=None, period_pk=None
):
    """
    ...
    if "parent" in nav_config:
        parent_title, parent_view, param_map = (
            nav_config["parent"]
            if len(nav_config["parent"]) == 3
            else (nav_config["parent"][0], nav_config["parent"][1], {})
        )
    ...
    for rel_title, rel_view, param_map in nav_config.get("related", []):
        ...
        if (
            rel_kwargs or not param_map
        ):  # Include if we have kwargs or no mapping needed
            try:
